...
export default function SubscriptionsScreen() {
  ...
  const totalMonthlyBudget = activeSubs.reduce((sum, s) => {
    return sum + (s.cycle === 'yearly' ? s.price / 12 : s.price);
  }, 0);
  ...
  const renderEmpty = () => (
    <View style={styles.empty}>
      <Text style={[styles.emptyTitle, { color: colors.neutral.textPrimary }]}>
        {Strings.subscriptions.emptyTitle}
      </Text>
      <Text style={[styles.emptySubtitle, { color: colors.neutral.textSecondary }]}>
        {Strings.subscriptions.emptySubtitle}
      </Text>
      <TouchableOpacity
      ...
      >
        ...
        <Text style={styles.addBtnText}>{Strings.subscriptions.addSubscription}</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <SafeScreenView>
      <View style={[styles.container, screenContainer, { backgroundColor: colors.neutral.bg }]}>
        ...
        <Text style={[styles.title, { color: colors.neutral.textPrimary }]}>{Strings.tabs.Subscriptions}</Text>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <SubscriptionItem
                ...
                year={selectedYear}
                month={selectedMonth}
                totalMonthlyBudget={totalMonthlyBudget || 1}
